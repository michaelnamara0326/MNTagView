...
也許你會想：「*在 SwiftUI 中手刻一個 Flow Layout 其實不難，為什麼還需要這個套件？*」

確實，單純的排版在 SwiftUI 中很容易實現。不過 `MNTagView` 希望提供的不只是排版，而是一個更完整的**整合方案**。這個套件主要希望能幫助開發者節省處理瑣碎細節的時間：

1.  **快速整合**：省去了從頭刻畫 Tag UI 的時間。圓角、邊框、背景色等常用樣式都已預先封裝，簡單設定參數即可使用。
2.  **狀態管理**：內建了選取狀態（Selection）與編輯模式（Remove Button）的邏輯，希望能讓開發者專注於業務邏輯，而不用分心處理這些互動狀態。
3.  **雙平台統一**：對於同時維護 SwiftUI 與 UIKit 的專案來說，這或許是一個方便的選擇。它提供了盡可能一致的 API 體驗，讓你在兩個框架間切換時能共用相似的設計邏輯。

如果你正好需要一個能快速上手、穩定且跨平台的標籤列表，希望 `MNTagView` 能為你的開發過程帶來一些便利。
